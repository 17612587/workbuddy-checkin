---
name: workbuddy-checkin
display_name: WorkBuddy 每日签到
display_name_en: WorkBuddy Daily Check-in
description: 使用当前 WorkBuddy 或 CodeBuddy CN 账号签到（含本次获得、累计获得、当前余额三字段）。用户要求签到、打卡、check in 时使用。
description_zh: 使用当前 WorkBuddy 或 CodeBuddy CN 账号完成每日签到，输出本次获得、累计获得、当前余额三个字段，支持配合 WorkBuddy 自动化每天定时签到并推送结果。
description_en: Automates daily check-in for the current WorkBuddy or CodeBuddy CN account and reports credits earned today, total credits and current balance. Works with scheduled automations.
category: 效率工具
version: 1.0.0
author: 17612587
---

# WorkBuddy 签到

用户明确要求签到后，直接运行，不要先读脚本：

```sh
python3 <skill-directory>/scripts/workbuddy-checkin.py
```

脚本会直接输出格式化的中文签到结果（含本次获得 / 累计获得 / 真实余额）。只回复一次：将脚本输出的中文结果原样返回给用户。

输出硬性要求（任何一条违反都视为严重错误）：
- 你的回复必须且只能是脚本输出结果，一个字都不能多、不能少、不能重复。
- 绝对禁止输出任何英文，禁止以 "Let me" / "I'll" / "Now let me" / "Now I" 开头的句子。
- 绝对禁止输出思考过程、状态说明、过程描述、"执行成功"之类的话。
- 绝对禁止读取或写入任何记忆文件（memory / 自动化记忆 / automation memory），不要把"读记忆""写执行摘要"这类动作说出口或写进回复。
- 结果只能出现一次，严禁重复打印。
- 不要泄露凭据、不要重试、不要添加任何额外解释。

## 自动签到

用户说"每天定时签到 / 设置自动签到 / 帮我每天打卡"时，直接创建一个 WorkBuddy 自动化（不要展示命令，不要在对话里让用户手写任何内容）：

- **名称**：`WorkBuddy 每日签到`
- **执行频率**：用户指定的时间；未指定则默认每天 `09:00`
- **执行命令**：

  ```sh
  python3 <skill-directory>/scripts/workbuddy-checkin.py
  ```

- **自动化 Prompt**（原样填入，不要改写）：

  ```
  执行签到任务。运行以下命令：

  <skill-directory>/scripts/workbuddy-checkin.py

  然后只做一件事：把脚本在终端输出的内容，从以「✅」开头的那一行开始、到「---」分隔符之前结束，原样作为企业微信消息发出（平台会自动在开头显示「自动化任务【WorkBuddy 每日签到】」作为头部，无需你再加）。

  硬性要求（违反任意一条都不行）：
  - 你的回复只能包含上述脚本结果，一个字都不能多、不能少、不能重复。
  - 绝对禁止输出任何英文，禁止以 "Let me" / "I'll" / "Now let me" / "Now I" 开头的句子。
  - 绝对禁止输出任何思考过程、任何状态说明、任何"执行成功"之类的话。
  - 绝对禁止读取或写入任何记忆文件（memory / 自动化记忆 / automation memory），不要把"读记忆""写执行摘要"这类动作说出口或写进回复。
  - 结果只能出现一次，严禁重复打印。
  - 不要自己添加标题行（头部由平台提供）。
  - 若脚本没有输出「✅」结果行（即报错），则只发送报错信息本身。
  ```

企业微信推送开关需用户在自动化设置里手动开启一次（脚本不含通知功能）。修改时间或开关请直接在 WorkBuddy 自动化设置里操作，本技能脚本不参与定时调度。
