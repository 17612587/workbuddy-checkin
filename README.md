...
如需每日自动签到 + 企业微信通知推送，请在 WorkBuddy 中创建自动化：

1. 打开 WorkBuddy → 左侧「自动化」→ 新建自动化
2. 设置执行频率为每天（建议 `09:00`）
3. 执行命令填写：
   ```sh
   python3 <skill-directory>/scripts/workbuddy-checkin.py
   ```
4. **手动开启「推送到自动化企微通知 bot」开关**（默认关闭，需用户自行开启）
5. Prompt 填写：将脚本输出的中文结果原样作为推送消息发送

> ⚠️ 企业微信通知需要在自动化设置中**手动开启**「推送到自动化企微通知 bot」开关，脚本本身不包含任何推送逻辑。
